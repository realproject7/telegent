...
`/etc/systemd/system/telegent-broker.service`:

```ini
[Unit]
Description=Telegent managed tunnel broker
...
sudo systemctl enable --now telegent-broker
sudo systemctl status telegent-broker
journalctl -u telegent-broker -f
```
